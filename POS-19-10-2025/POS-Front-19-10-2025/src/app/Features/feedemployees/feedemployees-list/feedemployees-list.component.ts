...
@Component({
  selector: "app-feedemployees-list",
  templateUrl: "./feedemployees-list.component.html",
  styleUrls: ["./feedemployees-list.component.scss"]
})
export class FeedemployeesListComponent extends imp.GeneralGrid implements imp.OnInit {
  //#region Declartions
  [key: string]: any;
  ...
  //#region Constructor
  constructor(
    public foodplanser: imp.FoodPlanService,
  ...
  initializeobjects(): void {
    this.responseobj = {};
    this.service = this.foodplanser;
    this.showEdit = false;
    this.showDelete = false;
    this.showNew = false;
    this.showView = false;
    this.showPrint = false;
    this.RouteName = "/Feedemployees";
    this.languageSerService.currentLang.subscribe((lan) => (this.language = lan));
    this.translate.use(this.language);
  }

  onResourceMealsEdit(event: any): void {
    const rowData = this.grid.getRowInfo(event.target).rowData as any;
    rowData.showEdit = this.showEdit;
    this.router.navigateByUrl("/Feedemployees/Connectmeals", rowData);
  }

  onResourceEmployeeEdit(event: any): void {
    const rowData = this.grid.getRowInfo(event.target).rowData as any;
    rowData.showEdit = this.showEdit;
    this.router.navigateByUrl("/Feedemployees/Connecemployees", rowData);
  }
  //#endregion
}
