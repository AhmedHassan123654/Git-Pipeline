...
export class InventoryComponent extends imp.general implements imp.OnInit {
  [key: string]: any;
  ...
  constructor(
    public inventoryService: imp.InventoryService,
  ...
  initializeobjects(): void {
    this.responseobj = {};
    //this.inventorytype= this.router.url.replace(/\//g, '');
    this.service = this.inventoryService;
    this.responseobj.Type = 1; //this.inventorytype =="stockinventory"? 1 :2;
  ...
  initiateData() {
    ...
    this.responseobj.StockDocumentId = this.responseobj.StockList[0].FlagValue;
    // this.responseobj.StockId = this.responseobj.StockList[0].StockId;
  ...
  downloadStockItems() {
    result: [];
    this.inventoryService.downloadStockItems(this.responseobj).subscribe((res) => {
